...
public class Leer {
    ...
    static public Date pedirDateSQL(final String texto) {
        BufferedReader dataIn = new BufferedReader(new InputStreamReader(System.in));
        Date dato = null;
        boolean error = true;
        String datoT = "";
        while (error) {
            try {
                mostrarEnPantalla(texto);
                datoT = dataIn.readLine();
                Date utilDate = new SimpleDateFormat("dd-MM-yyyy").parse(datoT);
                dato = new Date(utilDate.getTime()); // Convertir java.util.Date a java.sql.Date
                error = false;
            } catch (IOException e) {
                mostrarEnPantalla("Vuelve a introducir el dato, por favor. ");
                error = true;
